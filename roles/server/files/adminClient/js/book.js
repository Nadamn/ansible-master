...
bookAddBtn.addEventListener("click", function (evt) {
  ...
  xhttp.open("POST", "http://192.168.0.50:5000/admin/books");
...
bookEditBtn.addEventListener("click",(evt)=>{
  ...
  xhttp.open("POST", `http://192.168.0.50:5000/admin/books/${id}/edit`);
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send(JSON.stringify({
    "picture":localStorage.getItem("current_image_name"),
...
function listBooks() {
  ...
  xhttp.open("GET", "http://192.168.0.50:5000/admin/books");
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send()
};
