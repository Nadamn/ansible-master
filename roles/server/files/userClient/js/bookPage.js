...
function fillBookPage() {
  ...
  xhttp.open("POST", `http://192.168.0.50:5000/books/${bookId}`);
...
function showReviews() {
  ...
  xhttp.open("GET", `http://192.168.0.50:5000/books/${bookId}/comments`);
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send();
...
function AddReview() {
  ...
  xhttp.open("POST", `http://192.168.0.50:5000/books/${bookId}/comments`);
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send(JSON.stringify({
    comment: review,
    user_id: userId
